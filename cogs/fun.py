...
class Fun(commands.Cog):
    ...
    async def dice(self, ctx, sides: int = 6, quantity: int = 1):
        ...
        for num in numbers:
            results += str(num) + ", "
        if quantity == 1:
            embed=discord.Embed(title=f"{get(emojis, name='dice')} Dice Roll Results", description=f"You rolled **`{results[:-2]}`**!, color=0x36393f)
        else:
            embed=discord.Embed(title=f"{get(emojis, name='dice')} Dice Roll Results", description=f"You rolled these values: ```\n{results[:-2]}\n```That leads to a total of **{str(sum(numbers))}**!", color=0x36393f)
        await ctx.send(embed=embed)
