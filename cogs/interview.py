...
class Interview(commands.Cog):
    ...
    async def interview(self, ctx):
        ...
        async def waitUntilDone():
            ...
            await self.bot.wait_for('message', check=lambda message: message.author == ctx.author and message.content == "[DONE]") # Wait until the interviewee says [DONE]
        
        await newInt.send("Welcome to this interview session!")
        while True:
            await newInt.send("Before we begin, please complete this captcha to proceed...")
            captchaString = ''.join((random.choices((string.ascii_lowercase + string.ascii_uppercase + string.digits).replace("v", "").replace("o", "").replace("u", "").replace("w", "").replace("x", "").replace("z", "").replace("c", "").replace("l", "").replace("s", ""), k=random.randint(4, 6)))) # Generate random captcha string
            ...
            await newInt.send("""Use `a!newCaptcha` to generate a new captcha. The lowercase letters `v`, `o`, `u`, `w`, `x`, `z`, `c`, `s`, and `l` do not appear.""")
            print(captchaString)
        ...
        await newInt.send("Please note that all messages sent in this channel will be logged for future reference.")
        msg = await newInt.send(f"""If you agree, please choose from the following list by reacting to the message:
...
:no_entry: **__DO NOT REACT YET__**""")
        ...
        await msg.add_reaction("🇦")
        await msg.add_reaction("🇧")
        await msg.edit(content=f"""If you agree, please choose from the following list by reacting to the message:
...
:white_check_mark: **__REACT NOW__**""")
        ...
        if payload.emoji.name == '🇭':
            ...
            await newInt.send("You may raise questions now. Otherwise, type \"[BEGIN]\" to begin the interview process.")
            await self.bot.wait_for('message', check=lambda message: message.author == ctx.author and message.content == "[BEGIN]") # Wait until the interviewee says [BEGIN]
            await newInt.send("Firstly, please tell us your Minecraft username.")
            await waitUntilDone()
            await newInt.send("What time zone are you situated in? Please answer *in the form of **UTC** offset*, e.g. UTC+8, UTC+3:30")
            await waitUntilDone()
            await newInt.send("What is the Java Edition **IP** of the Aurum Minecraft Network Main/ Parkour Server?\n\nA. bedrock.atpcity.ga\nB. java.atpcity.ga\nC. be.atpcity.ga\nD. parkour.atpcity.ga")
            await waitUntilDone()
            await newInt.send("What is the command to go to the hub in the Main Server?")
            await waitUntilDone()
            await newInt.send("If a player from Bedrock Edition joins the Main Server, which symbol will be prepended before their username?\n\nA. Asterisk *\nB. Dollar symbol $\nC. Hashtag #\nD. Ampersand &")
            await waitUntilDone()
            await newInt.send("""Situation: a player with the username `JebPlayz` needs help, and approaches you. Assume that you are a helper in Aurum Minecraft Network, and your username is `BobbyPlayz`.""")
            await newInt.send("1. He has got the \"Unable to connect to world.\" issue. What would you do to help him troubleshoot?")
            await waitUntilDone()
            await newInt.send("""2. He wants to teleport to you. What command would you use to teleport him to you?\n***DO NOT*** use any entity selectors (`@s` `@e` `@a` `@r` `@p`).""")
            await waitUntilDone()
            await newInt.send("3. He got a broken pipe issue. You suggest him to open a ticket to get the issue solved. Which Discord channel shall he go to open a ticket? Mention the channel.")
            await waitUntilDone()
            await newInt.send("Why do you want to become a helper? Please briefly explain.")
            await waitUntilDone()
            ...
            await newInt.send("Firstly, please tell us your Minecraft username.")
            await waitUntilDone()
            await newInt.send("Please briefly explain why would you like to become a Discord Moderator.")
            await waitUntilDone()
            await newInt.send("What would you do for the server, if you successfully become a Discord Moderator?")
            await waitUntilDone()
            await newInt.send("The interview process has ended. Please raise any questions that you have now, otherwise, type \"[END]\" to archive this channel and await processing.")
            await self.bot.wait_for('message', check=lambda message: message.author == ctx.author and message.content == "[END]") # Wait until the interviewee says [END]
            await newInt.set_permissions(ctx.author, read_messages=True, send_messages=False)
            ...
            await newInt.send("Firstly, please tell us your Minecraft username.")
            await waitUntilDone()
            await newInt.send("Please briefly explain why would you like to become a Minecraft Moderator.")
            await waitUntilDone()
            await newInt.send("What would you do for the server, if you successfully become a Minecraft Moderator?")
            await waitUntilDone()
            await newInt.send("The interview process has ended. Please raise any questions that you have now, otherwise, type \"[END]\" to archive this channel and await processing.")
            await self.bot.wait_for('message', check=lambda message: message.author == ctx.author and message.content == "[END]") # Wait until the interviewee says [END]
            await newInt.set_permissions(ctx.author, read_messages=True, send_messages=False)
            ...
            await newInt.send("Firstly, please tell us your Minecraft username.")
            await waitUntilDone()
            await newInt.send("Please briefly explain why would you like to become a Discord + Minecraft Moderator.")
            await waitUntilDone()
            await newInt.send("What would you do for the server, if you successfully become a Discord + Minecraft Moderator?")
            await waitUntilDone()
            await newInt.send("The interview process has ended. Please raise any questions that you have now, otherwise, type \"[END]\" to archive this channel and await processing.")
            await self.bot.wait_for('message', check=lambda message: message.author == ctx.author and message.content == "[END]") # Wait until the interviewee says [END]
            await newInt.set_permissions(ctx.author, read_messages=True, send_messages=False)
            ...
            await newInt.send("You may raise questions now. Otherwise, type \"[BEGIN]\" to begin the interview process.")
            await self.bot.wait_for('message', check=lambda message: message.author == ctx.author and message.content == "[BEGIN]") # Wait until the interviewee says [BEGIN]
            await newInt.send("Although we would very love to have you on board already, it is necessary for us to prove that you have some programming abilities. Therefore, all applicants must solve a programming problem to proceed.")
            await newInt.send("""Create a function called `fizz` that takes in five arguments:
...
Use any programming language.""")
            await newInt.send("""Example input in Python:```py
...
ATP
City```""")
            await newInt.send("""Python starter code ```py
...
fizz(
    # Parameters here
            ...
            await newInt.send("""JavaScript starter code ```js
...
fizz(
    // Parameters here
            ...
            await newInt.send("""Java starter code ```java
public class Main {
    ...
    public static void main(String[] args) {
        fizz(
            ...
            await newInt.send("""C++ starter code ```cpp
#include <iostream>
#include <string>
using namespace std;
...
int main() {
    fizz(
        // Parameters here
            ...
            await newInt.send("When you are done, send the code in a formatted way.\nMore info: https://gist.github.com/matthewzring/9f7bbfd102003963f9be7dbcf7d40e51#syntax-highlighting")
            await waitUntilDone()
            await newInt.send("Now, please tell us your Minecraft username.")
            await waitUntilDone()
            await newInt.send("Please briefly explain why would you want to become a developer and help us.")
            await waitUntilDone()
            await newInt.send("The interview process has ended. Please raise any questions that you have now, otherwise, type \"[END]\" to archive this channel and await processing.")
            await self.bot.wait_for('message', check=lambda message: message.author == ctx.author and message.content == "[END]") # Wait until the interviewee says [END]
            await newInt.set_permissions(ctx.author, read_messages=True, send_messages=False)
            ...
            await newInt.send("Firstly, please tell us your Minecraft username.")
            await waitUntilDone()
            await newInt.send("What image editing programs can you use?\ne.g. Adobe Photoshop, paint.net, Adobe Illustrator, GIMP")
            await waitUntilDone()
            await newInt.send("If applicable, describe your style of art/ images you make\ne.g. minimalistic, cartoony")
            await waitUntilDone()
            await newInt.send("If you want, you may send some fantastic work of yours to us now! Note that this is not mandatory.")
            await newInt.send("The interview process has ended. Please raise any questions that you have now, otherwise, type \"[END]\" to archive this channel and await processing.")
            await self.bot.wait_for('message', check=lambda message: message.author == ctx.author and message.content == "[END]") # Wait until the interviewee says [END]
            await newInt.set_permissions(ctx.author, read_messages=True, send_messages=False)
            await newInt.edit(category=get(guild.channels, id=817798276366991371), name=f"int-ar-{intID}")
